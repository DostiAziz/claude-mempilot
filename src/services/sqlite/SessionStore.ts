...
export class SessionStore {
  ...
  private ensureFeaturesTables(): void {
    const applied = this.db.prepare('SELECT version FROM schema_versions WHERE version = ?').get(64) as SchemaVersion | undefined;
    if (applied) return;
    ...
    try {
      ...
      if (!cols.some(c => c.name === 'feature_id')) {
        this.db.run('ALTER TABLE observations ADD COLUMN feature_id INTEGER REFERENCES features(id) ON DELETE SET NULL');
      }
      ...
      this.db.run("CREATE INDEX IF NOT EXISTS idx_todos_open ON todos(feature_id) WHERE status = 'open'");
      this.db.run('CREATE INDEX IF NOT EXISTS idx_observations_branch_feature ON observations(branch_name, feature_id)');

      this.db.prepare('INSERT INTO schema_versions (version, applied_at) VALUES (?, ?)').run(64, new Date().toISOString());
